...
import { faHtml5, faCss3, faReact, faGithub, faPython, faJava, faAndroid } from '@fortawesome/free-brands-svg-icons';
...
export default function Projects() {
  const [viewAI, setAI] = useState(false)
  const [viewGG, setGG] = useState(false)
  const [viewVLA, setVLA] = useState(false)
    ...
    <>
      <Container style={{ marginTop: 35 }}>
        ...
        <Typography style={{ fontWeight: 600 }} variant='h6'>
          Responsible AI Design Assistant (September - December 2020)
          <Tooltip title={!viewAI ? "Show more" : "Show less"} placement="left" TransitionComponent={Zoom}>
            <IconButton onClick={() => setAI(!viewAI)} aria-label={!viewAI ? "Show more" : "Show less"}>{viewAI ? <ExpandLessIcon/> :<ExpandMoreIcon/> }</IconButton>
          </Tooltip>
        </Typography>
        ...
        <Divider />
        <Typography style={{ fontWeight: 600 }} variant='h6'>
          GufyGuber (January - April 2020)
          <Tooltip title={!viewGG ? "Show more" : "Show less"} placement="left" TransitionComponent={Zoom}>
            <IconButton onClick={() => setGG(!viewGG)} aria-label={!viewGG ? "Show more" : "Show less"} >{viewGG ? <ExpandLessIcon/> :<ExpandMoreIcon/> }</IconButton>
          </Tooltip>
          </Typography>
        ...
        <Divider />
        <Typography style={{ fontWeight: 600 }} variant='h6'>
          VLA Radio Observations of Blazar TXS 0506+056 (January - April 2018)
          <Tooltip title={!viewVLA ? "Show more" : "Show less"} placement="left" TransitionComponent={Zoom}>
            <IconButton onClick={() => setVLA(!viewVLA)} aria-label={!viewVLA ? "Show more" : "Show less"}>{viewVLA ? <ExpandLessIcon/> :<ExpandMoreIcon/> }</IconButton>
          </Tooltip>
        </Typography>
      ...
      <Container style={{ marginTop: 70, paddingBottom: 70 }}>
        <Typography style={{ fontWeight: 600, marginBottom: 35 }} variant='h4' color='primary'>Skills</Typography>
        <Grid container spacing={3}>
          <Grid item className="skills" xs={12} sm={6} md={3}>
            <FontAwesomeIcon icon={faHtml5} size="3x"/><Typography>HTML 5</Typography>
          </Grid>
          <Grid item className="skills"xs={12} sm={6} md={3}>
            <FontAwesomeIcon icon={faCss3} size="3x"/><Typography>CSS 3</Typography>
          </Grid>
          <Grid item className="skills" xs={12} sm={6} md={3}>
            <FontAwesomeIcon icon={faReact} size="3x"/><Typography>React</Typography>
          </Grid>
          <Grid item className="skills" xs={12} sm={6} md={3}>
            <FontAwesomeIcon icon={faGithub} size="3x"/><Typography>GitHub</Typography>
          </Grid>
          <Grid item className="skills" xs={12} sm={6} md={3}>
            <FontAwesomeIcon icon={faPython} size="3x"/><Typography>Python</Typography>
          </Grid>
          <Grid item className="skills" xs={12} sm={6} md={3}>
            <FontAwesomeIcon icon={faJava} size="3x"/><Typography>Java</Typography>
          </Grid>
          <Grid item className="skills" xs={12} sm={6} md={3}>
            <FontAwesomeIcon icon={faAndroid} size="3x"/><Typography>Android</Typography>
          </Grid>
          <Grid item className="skills" xs={12} sm={6} md={3}>
            <FontAwesomeIcon icon={faDatabase} size="3x"/><Typography>SQL/NoSql </Typography>
          </Grid>
        </Grid>
      </Container>
    </>
